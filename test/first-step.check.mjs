...
test('seam summary: three quarters and up holds, under a half sags, the rest in between, chain order inside each bucket, unrated seams skipped', () => {
  ...
  assert.equal(F.seamText(r), 'Тримаються: Зрозуміла задача (28 з 28), Робота по кроках (28 з 28). Посередині: Потрібна інформація (14 з 28). Просідають: Перевірка результату (7 з 28), Корисні висновки (1 з 4).');
  ...
  assert.match(F.seamText(F.calculate(answers(F, 4))), /^Тримаються: .*Корисні висновки \(28 з 28\)\.$/);
  assert.doesNotMatch(F.seamText(F.calculate(answers(F, 4))), /Посередині|Просідають/);
  assert.match(F.seamText(F.calculate(answers(F, 0))), /^Просідають: /);
  const onlyOne = answers(F, 'none'); onlyOne.q15 = 3;
  assert.equal(F.seamText(F.calculate(onlyOne)), 'Тримаються: Робота по кроках (3 з 4).');
...
test('final Markdown adds the seam summary between the weakest seam and the first habit, answers without extra columns', () => {
  ...
  assert.match(md, /## Найслабший стик\n\nПеревірка результату \(Verification\), 7 з 28\. У мінікурсі це урок 5\. [^\n]+\n\n## Сильні й слабкі стики\n\nТримаються: Зрозуміла задача \(26 з 28\), Потрібна інформація \(28 з 28\), Робота по кроках \(24 з 24\), Корисні висновки \(28 з 28\)\. Просідають: Перевірка результату \(7 з 28\)\.\n\n## Перша звичка по ланцюгу\n\n/);
