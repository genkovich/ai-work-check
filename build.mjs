...
final = must(final,
  ...
  '<meta name="description" content="Необовʼязкова анкета мінікурсу: 35 запитань із підказками про розробку з агентами. Допомагає обрати тему для наступної практики.">',
  'description');
...
final = must(final,
  ...
  '<p class="intro">Тридцять пʼять запитань допоможуть переглянути свої звички в розробці й обрати наступну тему для практики. Тут по сім запитань на кожну з пʼяти груп. Проходити всю анкету для завершення мінікурсу необовʼязково.</p>',
  'intro');
final = must(final, "<p class=\"note\">Як вибирати рівень? Оціни, як часто ти так робиш, коли така нагода є: від «Ніколи» до «Завжди». 3 або 4 став лише тоді, коли міг би показати це з останньої задачі: файл, тест, запис. Якщо такої ситуації ще не було, познач це окремо під рядком, і запитання не ввійде в підрахунок. Так можна відповісти хоч на всі.</p>", "<p class=\"note\">Як вибирати рівень? Оціни частоту у відповідних ситуаціях: від «Ніколи» до «Завжди». Високу оцінку підкріплюй кількома прикладами з різних задач. Якщо досвіду ще немає, познач це під запитанням: воно не ввійде в підрахунок. Так можна відповісти хоч на всі 35 запитань.</p>", 'final frequency note');
